...
class VelbusDiscoveryProtocol(asyncio.DatagramProtocol):
    ...
    def datagram_received(self, data: Union[bytes, Text], addr: Address):
        ...
        try:
            ...
            return
        if all(key in json_data for key in ('message', 'hostname', 'model', 'id', 'velbus_port', 'velbus_auth')):
            res = {
                'address': addr[0],
...
coro = loop.create_datagram_endpoint(
    lambda: VelbusDiscoveryProtocol(('192.168.1.255', 32767)), local_addr=('0.0.0.0', 1900))
